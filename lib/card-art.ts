...
function referenceArtwork({theme, side, name = '', email = '', qr = '', art, colors, copy = {}}: Parameters<typeof cardArtwork>[0]) {
  ...
  const back = portrait
    ...
    : centered
      ? `${qrBlock}${text(copy.name ?? (name || 'Nome do titular'),505,425,34,810,600,'middle')}${text(copy.email ?? (email || 'Email do titular'),505,480,25,810,400,'middle')}${text('',65,75,23,660,600)}${nfc(900,530)}`
      : `${text('',tx,100,25,650,600)}${text(copy.name ?? (name || 'Nome do titular'),tx,390,40,625,600)}${text(copy.email ?? (email || 'Email do titular'),tx,447,25,625)}${text(copy.action ?? 'Aproxime ou leia o QR',tx,520,21,625)}${qrBlock}${nfc(890,65)}`;
